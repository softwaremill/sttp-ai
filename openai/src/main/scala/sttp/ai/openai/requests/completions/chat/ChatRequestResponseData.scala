...
object ChatRequestResponseData {
  ...
  /** @param content
    *   The contents of the message. Can be optional because some providers (vertex AI), sometimes return response without content (in case
    *   of tool calling)
    * @param refusal
    ...
    *   The reasoning/chain-of-thought produced by reasoning models served through OpenAI-compatible APIs. Populated from
    *   `reasoning_content` (DeepSeek, Qwen, vLLM, Ollama), falling back to `reasoning` (OpenRouter, Groq, xAI).
    */
  case class Message(
      content: String = "",
