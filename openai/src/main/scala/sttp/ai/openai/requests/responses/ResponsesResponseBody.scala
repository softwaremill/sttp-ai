...
object ResponsesResponseBody {
  ...
  object InputItem {
    ...
    object OutputContent {
      case class OutputText(text: String, annotations: List[Json] = List.empty) extends OutputContent
    ...
    case class InputMessage(content: Either[String, List[InputContent]], role: String, status: Option[String] = None) extends Message
    ...
    case class FunctionCallOutput(callId: String, output: String, id: Option[String] = None, status: Option[String] = None)
        extends InputItem
  ...
  object OutputItem {
    ...
    case class FunctionCall(arguments: String, callId: String, name: String, id: String, status: String) extends OutputItem
    ...
    case class WebSearchCall(action: WebSearchCall.Action, id: String, status: String) extends OutputItem
    ...
    /** An output item whose `type` this version of the library does not model. The Responses API emits more item types than are modelled
      ...
      */
    case class Unknown(`type`: String, raw: Json) extends OutputItem

    /** The item `type`s modelled above. Anything else decodes to [[Unknown]]; a new case must be added here too. */
    ...
    case class FileSearchResult(attributes: Map[String, Json], fileId: String, filename: String, score: Double, text: String)
  ...
  object OutputContent {
    ...
    // `bytes` and `topLogprobs` default to empty: streamed `output_text` content parts omit them.
    case class TopLogProb(bytes: List[Byte] = Nil, logprob: Double, token: String)
    case class LogProb(bytes: List[Byte] = Nil, logprob: Double, token: String, topLogprobs: List[TopLogProb] = Nil)

    case class OutputText(annotations: List[Annotation], text: String, logprobs: Option[List[LogProb]] = None) extends OutputContent
    case class Refusal(refusal: String) extends OutputContent

    /** A reasoning-text content part. Emitted as the `part` of `response.content_part.added` / `.done` on reasoning items. */
    case class ReasoningText(text: String) extends OutputContent

    /** A content part whose `type` this version of the library does not model. `response.content_part.added` / `.done` are themselves
      * modelled event types, so without this fallback a part type added by OpenAI after this release (a future `output_audio`, say) would
      * fail decoding and error the whole stream. Carries the verbatim part JSON.
      */
    case class Unknown(`type`: String, raw: Json) extends OutputContent

    /** The content-part `type`s modelled above. Anything else decodes to [[Unknown]]; a new case must be added here too. */
    val KnownTypes: Set[String] = Set("output_text", "refusal", "reasoning_text")
  ...
  /** @param cachedTokens
    ...
    */
  case class InputTokensDetails(cachedTokens: Option[Int], cacheWriteTokens: Option[Int] = None)
