...
/** A single server-sent event emitted by a streamed Responses API call.
  ...
  * e.g. `response.output_text.delta` becomes [[ResponsesStreamEvent.OutputTextDelta]].
  *
  * Decode-only: these events are only ever received, never sent. Dispatch is a lookup in the wire `type` -> decoder registry
  * `OpenAIDerivedCodecs.responsesStreamEventDecoders`, the single source of truth for which events this release models, so a new case must
  * be registered there or it will always decode as [[ResponsesStreamEvent.Unknown]]. A `type` with no entry decodes to
...
object ResponsesStreamEvent {
  ...
  /** Handling observed mismatch between the wire model format and documented one:
    * https://developers.openai.com/api/reference/resources/beta/subresources/responses/streaming-events#response.function_call_arguments.done
  ...
  /** An event whose `type` has no entry in `OpenAIDerivedCodecs.responsesStreamEventDecoders`, e.g. one introduced by OpenAI after this
    * release. Carries the verbatim frame so it can be inspected or logged; `sequenceNumber` is `-1` if the frame carried none.
    */
