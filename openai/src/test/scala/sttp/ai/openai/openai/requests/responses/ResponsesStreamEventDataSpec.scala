...
class ResponsesStreamEventDataSpec extends AnyFlatSpec with Matchers with EitherValues {
  ...
  "Every type in ResponsesStreamEvent.KnownTypes" should "decode to a concrete case carrying that type" in
    ResponsesStreamEvent.KnownTypes.foreach { eventType =>
  ...
  it should "cover every documented event type" in {
    ResponsesStreamEvent.KnownTypes should have size 58
    ResponsesStreamEventFixture.minimalByType.keySet shouldBe ResponsesStreamEvent.KnownTypes
  }
