...
/** JSON frames for the streamed Responses API events, in the shape OpenAI sends them.
  *
  * [[minimalByType]] holds one minimal-but-valid frame per modelled event type; `ResponsesStreamEventDataSpec` walks the keys of
  * `OpenAIDerivedCodecs.responsesStreamEventDecoders` against it, which is what keeps the model, the decoder and this fixture from drifting
  * apart.
  */
object ResponsesStreamEventFixture {
  ...
  private def parsed(json: String): Json =
    parse(json).fold(failure => throw new IllegalArgumentException(s"invalid fixture JSON: $json", failure), identity)
  ...
  /** A content-part `type` this library does not model, so it must decode to `OutputContent.Unknown`. */
  val jsonUnknownPart: String = """{"type":"output_audio","data":"aGk=","transcript":"Hi"}"""
  ...
  val jsonCompletedWithFullUsage: String = {
    ...
    val withUsage = parsed(ResponsesFixture.jsonResponseBasic).deepMerge(parsed(usage))
    frame("response.completed", s""""response":${withUsage.noSpaces}""")
  }
