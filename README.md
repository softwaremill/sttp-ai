...
```sbt
"com.softwaremill.sttp.ai" %% "openai" % "0.6.0"
```
...
```sbt
"com.softwaremill.sttp.ai" %% "claude" % "0.6.0"

// For streaming support, add one or more (these modules are shared across OpenAI, Claude, and Gemini):
"com.softwaremill.sttp.ai" %% "fs2" % "0.6.0"    // cats-effect/fs2
"com.softwaremill.sttp.ai" %% "zio" % "0.6.0"    // ZIO
"com.softwaremill.sttp.ai" %% "akka" % "0.6.0"   // Akka Streams (Scala 2.13 only)
"com.softwaremill.sttp.ai" %% "pekko" % "0.6.0"  // Pekko Streams
"com.softwaremill.sttp.ai" %% "ox" % "0.6.0"    // Ox direct-style (Scala 3 only)
```
...
```sbt
"com.softwaremill.sttp.ai" %% "gemini" % "0.6.0"
...
Then head to the [documentation](https://sttp-ai.softwaremill.com) for usage examples: OpenAI, Claude, and Gemini clients, streaming, structured outputs, tool calling, and the agent loop — plus the `testkit` module for testing agents without calling a paid API.
