...
private[agent] object MacroSupport {
  ...
  /** Renders a type from bare symbol names (recursing into type arguments and union branches) rather than via `tpe.show`. When a macro is
    * exercised via scala.compiletime.testing.typeCheckErrors (see AgentToolsDeriveErrorsSpec / ResponseSchemaDerivedUnionSpec), types can
    * be declared inside the typechecked snippet; `.show`ing such a type after other symbol lookups (methodMembers, getAnnotation, ...) have
