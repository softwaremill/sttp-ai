...
import sttp.ai.jev.{Choice, ChoiceAnswer, Noul, NoulAnswer, Question, Score, ScoreAnswer}
...
/** Decodes answers strictly against the questions that produced them: the answer `type` must match the question, a choice and every
  * probability key must be one of the options, and every level must have a probability. Extra keys (including the score `legend`) are
  * ignored.
  */
private[jev] object AnswerDecoder:
  /** Matches the answer keyed `i` in `answers` to `questions(i)`. */
  def decodeAll[A](questions: Seq[Question[A]], answers: JsonObject): Decoder.Result[Seq[A]] =
  ...
  private def decode[A](question: Question[A], answer: Json): Decoder.Result[A] =
  ...
  private def decodeTyped[A](question: Question[A], cursor: HCursor): Decoder.Result[A] = question match
    case _: Noul           => cursor.get[Double]("noul").map(NoulAnswer(_))
    case choice: Choice[o] => decodeChoice(choice, cursor)
    case score: Score      => decodeScore(score, cursor)
  ...
  private def decodeScore(question: Score, cursor: HCursor): Decoder.Result[ScoreAnswer] =
    for
