...
class GeminiSyncClientSpec extends AnyFlatSpec with Matchers {
  ...
  it should "throw the mapped exception on API errors" in {
    ...
    an[GeminiException.AuthenticationException] should be thrownBy
      clientReturning(error, StatusCode.Unauthorized).createInteraction(InteractionRequest.simple("gemini-2.5-flash-lite", "hi"))
  }

  it should "decode structured output via createInteractionAs" in {
    val result = clientReturning(completedResponse)
      .createInteractionAs[WeatherReport](InteractionRequest.simple("gemini-2.5-flash-lite", "weather in Warsaw"))
